...
data = np.load(os.path.join('npz', '202072563.npz'))['data'][()]
...
ax[0,1].plot(P(time), fwhite_sff, 'r.', alpha = 0.3)
ax[0,2].plot(P(time), fwhite, 'b.', alpha = 0.3)

ax[1,0].plot(S(time), flux, 'k.', alpha = 0.3)
ax[1,1].plot(S(time), fwhite_sff, 'r.', alpha = 0.3)
ax[1,2].plot(S(time), fwhite, 'b.', alpha = 0.3)

ax[0,0].set_xlim(-0.25, 0.25)
ax[0,0].set_ylim(0.75, 1.05)
ax[0,0].yaxis.set_ticks([0.75, 0.8, 0.85, 0.9, 0.95, 1.0])
ax[0,0].set_title('Raw SAP Flux', fontsize = 20)
ax[0,1].set_title('K2SFF Flux', fontsize = 20)
ax[0,2].set_title('EVEREST Flux', fontsize = 20)
