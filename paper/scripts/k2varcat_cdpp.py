...
for campaign in range(0,7):
  ...
  if not os.path.exists(os.path.join('CDPP', 'k2varcat_C%02d.tsv' % campaign)):
  ...
  with warnings.catch_warnings():
    ...
    done = np.loadtxt(os.path.join('CDPP', 'k2varcat_C%02d.tsv' % campaign), dtype = float)
  if len(done):
  ...
  with open(os.path.join('CDPP', 'k2varcat_C%02d.tsv' % campaign), 'a') as outfile:
    ...
    for star in stars:
      ...
      # Get the cdpp
      try:
        s = kplr.K2VARCAT(star)
      except HTTPError:
        print("{:>09d} {:>15.3f} {:>15.3f}".format(star, 0, 0), file = outfile)
        continue
      rms = RMS(s.flux / np.nanmedian(s.flux), remove_outliers = True)
      flux_sv2 = s.flux - savgol_filter(s.flux, 49, 2) + np.nanmedian(s.flux)
