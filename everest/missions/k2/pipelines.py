  ...
  with open(file, 'a', 1) as outfile:
    ...
    for EPIC in stars:
      ...
      try:
        ...
        flux = np.delete(flux, mask)
        cdpp = CDPP(flux)
      except (urllib.error.HTTPError, urllib.error.URLError, TypeError, ValueError, IndexError):
...
def get_outliers(campaign, pipeline = 'everest2', sigma = 5):
  '''
  Computes the number of outliers for a given `campaign` and a given `pipeline`.
  Stores the results in a file under "/missions/k2/tables/".
